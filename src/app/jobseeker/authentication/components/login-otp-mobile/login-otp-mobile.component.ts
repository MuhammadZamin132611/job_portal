...
Why: To verify the opt with the given mobile number address
*/
...
@Component({
  selector: 'app-login-otp-mobile',
  templateUrl: './login-otp-mobile.component.html',
  styleUrls: ['./login-otp-mobile.component.css']
})

export class LoginOtpMobileComponent implements OnInit {
  // Declare class properties to store variables
  otp: string;
  data = localStorage.getItem('sms');
  answer: string;
  ...
  constructor(private auth: LoginEmailService, private router: Router) {
    ...
    this.secondsms = localStorage.getItem("sms");
    this.firstthree = localStorage.getItem("sms");
    this.next = localStorage.getItem("sms");
    this.last = localStorage.getItem("sms");
  ...
  // Function to handle sign-in with SMS
  public async signIn() {
    this.resendtime = false;
    this.busy_.next(true);
    this.errorMessage_.next('');
    try {
      await this.auth.signInEmail(this.secondsms);
      this.resendtime = false;
      this.timer(1);
    } catch (err: any) {
